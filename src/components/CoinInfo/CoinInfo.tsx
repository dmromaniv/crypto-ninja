    ...
    <div className="flex flex-col gap-y-3">
      ...
      <p className="flex items-center gap-x-2 text-lg font-medium">
        {coin?.market_data?.current_price?.["usd"]
          ? formatCurrency({ number: coin.market_data.current_price["usd"] })
          : MESSAGES.EMPTY_VALUE}

        {coin?.market_data?.price_change_24h_in_currency?.["usd"] && (
          <ChangePercentage percentage={coin.market_data.price_change_24h_in_currency["usd"]} />
        )}
      </p>
      {coin?.description?.en && <p>{coin.description.en}</p>}
      <p>
        Market Capitalization:
        <span className="text-lg font-medium">
          {coin?.market_data?.market_cap?.["usd"]
            ? formatCurrency({ number: coin.market_data.market_cap["usd"], notation: "compact" })
            : MESSAGES.EMPTY_VALUE}
        </span>
      </p>
      <p>
        ...
        <span className="font-medium">
          {coin?.categories.length > 0 ? coin.categories.join(", ") : MESSAGES.EMPTY_VALUE}
        </span>
      </p>
    </div>
  );
